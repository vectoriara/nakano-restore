...
function parseWater($conf,$row){
	...
	$trg_day = substr($row["t100_date"],0,8);
	// print "t100_date => ".$row["t100_date"]."\n";
	$max__water_level_time = DateConvert3($trg_day.str_replace(":","",$ArySum[$trg_day]["55"]["water_1min_".$pnt_id]));
	...
	// print date("Y/m/d H:i:s",$check_time)." ".date("Y/m/d H:i:s",$curTIme)." 10min\n";
	if($curTIme==$check_time){
		$tmp["max__water_level"] = (float) $ArySum[$trg_day]["50"]["water_1min_".$pnt_id];
