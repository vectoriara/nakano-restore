...
$id_map = array(
	...
	"logical-00001" => array("pnt_id" => "002","type" => "rain"),	// 区役所
...
$params = [
    'TableName' => "iws-sls-rain-store-prod",
    'Item' => ""
];
...
$strSql = "SELECT * FROM t500_dat WHERE t500_sdt BETWEEN '".date("Y-m-d H:i:s",$start_time)."' AND '".date("Y-m-d H:i:s",$end_time)."';";
print $strSql."\n";
...
while($row){
	...
	foreach($AryID as $key => $val){
		if($val["pnt_id"]==$row["t500_pnt_id"]){
			$logical_id = $key."_rain_1min";
			break;
		}
		// 杉並と東京都対応
		if($key=="logical-20002"||$key=="logical-20003"){
			foreach($val["point"] as $pnt_id => $pnt_info){
				if($pnt_id==$row["t500_pnt_id"]){
					$logical_id = $key."_rain_1min_".$pnt_info["id"];
					break;
				}
			}
		}
	...
	print $row["t500_pnt_id"]." ".$logical_id."\n";

	$tmp = [];
	$tmp["rainId"] = $logical_id;
	$tmp["targetTime"] = strtotime($row["t500_sdt"]);
	$tmp["endTime"] = strtotime($row["t500_edt"]);
	$tmp["lastTime"] = strtotime($row["t500_last"]);
	$tmp["value"] = (float) $row["t500_value"];
	$tmp["max_rain_10min"] = (float) $row["t500_rain_10min"];
	$tmp["max_rain_10min_time"] = strtotime($row["t500_rain_10min_date"]);
	$tmp["max_rain_hour"] = (float) $row["t500_rain_hour"];
	$tmp["max_rain_hour_time"] = strtotime($row["t500_rain_hour_date"]);
	
	// print_r($tmp);
	$buf = json_encode($tmp,JSON_PRETTY_PRINT);
	print $buf."\n";
...
exit();
// {
//     "rainId": "logical-00005_rain_1min",
//     "targetTime": 1581813901, 2020-02-16 09:45:01
//     "endTime": 1581839462, 2020-02-16 16:51:02
//     "lastTime": 1581817862, 2020-02-16 10:51:02
//     "value": 3.5,
//     "max_rain_10min": 1,
//     "max_rain_10min_time": 1581816360, 2020-02-16 10:26:00
//     "max_rain_hour": 3,
//     "max_rain_hour_time": 1581817860 2020-02-16 10:51:00
// }
// {
//   "endTime": 1609308420, 2020-12-30 15:07:00
//   "lastTime": 1609286820, 2020-12-30 09:07:00
//   "max_rain_10min": 0.5,
//   "max_rain_10min_time": 1609287360, 2020-12-30 09:16:00
//   "max_rain_hour": 1.5,
//   "max_rain_hour_time": 1609288920, 2020-12-30 09:42:00
//   "rainId": "logical-00001_rain_1min",
//   "targetTime": 1609285380, 2020-12-30 08:43:00
//   "value": 1.5
// }
// {
//   "endTime": 1592328660, 2020-06-17 02:31:00
//   "lastTime": 1592307060, 2020-06-16 20:31:00
//   "max_rain_10min": 1,
//   "max_rain_10min_time": 1592307600, 2020-06-16 20:40:00
//   "max_rain_hour": 1,
//   "max_rain_hour_time": 1592310600, 2020-06-16 21:30:00
//   "rainId": "logical-20002_rain_1min_s2",
//   "targetTime": 1592307060, 2020-06-16 20:31:00
//   "value": 1
// }
...
//     [1] => 005
//     [t500_pnt_id] => 005
//     [2] => 2020-02-16 09:45:01
//     [t500_sdt] => 2020-02-16 09:45:01
//     [3] => 2020-02-16 16:51:02
//     [t500_edt] => 2020-02-16 16:51:02
//     [4] => 2020-02-16 10:51:02
//     [t500_last] => 2020-02-16 10:51:02
//     [5] => 3.5
//     [t500_value] => 3.5
//     [6] => 1.0
//     [t500_rain_10min] => 1.0
//     [7] => 2020-02-16 10:26:00
//     [t500_rain_10min_date] => 2020-02-16 10:26:00
//     [8] => 3.0
//     [t500_rain_hour] => 3.0
//     [9] => 2020-02-16 10:51:00
//     [t500_rain_hour_date] => 2020-02-16 10:51:00
// )
...
while($start_time<=$end_time){
	...
	if($wk_ym!=$date["year"].$date["month"]){
		...
		while($row){
			...
			if(trim($argv[3])=="logical-00018") $trg_day = substr($row["t000_date"],0,8);
			...
			foreach($AryID as $logical_id => $conf){
				...
				if($logical_id=="logical-00018"){
					print "t000_date => ".$row["t000_date"]." logical_id[".$logical_id."]\n";
				}else{
